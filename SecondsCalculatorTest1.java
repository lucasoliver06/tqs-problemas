...
class SecondsCalculatorTest1 {
	...
	void testSegonsFins() {
		...
		assertEquals(86399, sec.SegonsFins(1, 1, 1980, 23, 59, 59));
		assertEquals(662774400, sec.SegonsFins(1, 1, 2001, 0, 0, 0)); // Test acumulació any traspàs secular (2000)
		assertEquals(31622399, sec.SegonsFins(31, 12, 1980, 23, 59, 59)); //Test últim segon de l'any
	...
	void testDiesDelMes() {
		...
		// Partició equivalent: Mesos de 31 dies
		assertEquals(31, sec.getDiesDelMes(1, 2000));
		
		//Partició equivalent: Mesos de 30 dies		
		assertEquals(30, sec.getDiesDelMes(4, 2000));
		
		// Partició equivalent: Febrer (traspàs i no traspàs)
		assertEquals(29, sec.getDiesDelMes(2, 2000));
		assertEquals(28, sec.getDiesDelMes(2, 2100));
		
		// Particiones de equivalencia inválidas (límites)
		assertEquals(-1, sec.getDiesDelMes(13, 2000)); 
		assertEquals(-1, sec.getDiesDelMes(0, 2000));
		assertEquals(-1, sec.getDiesDelMes(-1,2000));
